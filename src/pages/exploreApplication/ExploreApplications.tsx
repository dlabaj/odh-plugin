...
const ExploreApplicationsInner: React.FC<ExploreApplicationsInnerProps> =
  React.memo(
    ({
      loaded,
      isEmpty,
                      ...
                      <Gallery className="odh-explore-apps__gallery" hasGutter>
                        {exploreComponents.map((c) => (
                          <OdhExploreCard
                            ...
                            enableOpen={
                              c.metadata.name === enableApp?.metadata.name
                            }
...
const ExploreApplications: React.FC = () => {
  ...
  const [selectedComponent, setSelectedComponent] =
    React.useState<OdhApplication>();
  const isEmpty = !components || components.length === 0;

  const updateSelection = React.useCallback(
    (selectedId?: string | null): void => {
      const selection = components.find(
        (c) => c.metadata.name && c.metadata.name === selectedId,
      );
